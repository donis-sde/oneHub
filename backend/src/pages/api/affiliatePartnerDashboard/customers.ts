...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<CustomerResponse | { error: string }>,
): Promise<void> {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    const url = `${baseUrl}/api/v1/Customer/GetAll?${queryParams.toString()}`;
    ...
    console.log(
      'Partnership service response - customers count:',
      data.Items?.length || 0,
      'total:',
      data.TotalCount,
      'filters:',
