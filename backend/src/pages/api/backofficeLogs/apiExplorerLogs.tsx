...
export default async function handler(
  ...
  try {
    await rbacMiddleware(req, res, async () => {
      ...
      if (otherQuery) {
        const { data, count } = await apiExplorerLogDao.list(paginator, filter);
        res.status(200).json({ logs: data, total: count });
      } else {
        const { data, count } = await apiExplorerLogDao.list(paginator);
        res.status(200).json({ logs: data, total: count });
      }
    });
  } catch (error) {
    console.error('Failed to fetch API Explorer logs', error);
    res.status(500).json({ error: 'Failed to fetch API Explorer logs' });
  }
}
