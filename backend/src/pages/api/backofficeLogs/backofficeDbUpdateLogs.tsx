...
const rbacMiddleware = createRbacMiddleware(
  rbacRules,
  BackofficeFeature.DB_UPDATE_LOGS,
);
...
export default async function handler(
  ...
  try {
    await rbacMiddleware(req, res, async () => {
      ...
      console.log('second in sdfjf', filter);
      ...
      if (otherQuery) {
        const { data, count } = await backofficeDbUpdateLogsDao.list(
          paginator,
          filter,
        );
        res.status(200).json({ logs: data, total: count });
      } else {
        const { data, count } = await backofficeDbUpdateLogsDao.list(paginator);
        res.status(200).json({ logs: data, total: count });
      }
      // const { data, count } = await backofficeDbUpdateLogsDao.list(paginator,filter);
    });
  } catch (error) {
