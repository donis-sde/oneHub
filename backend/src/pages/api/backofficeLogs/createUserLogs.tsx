...
const rbacMiddleware = createRbacMiddleware(
  rbacRules,
  BackofficeFeature.CREATE_USER_LOGS,
);
...
export default async function handler(
  ...
  try {
    await rbacMiddleware(req, res, async () => {
      const { createUserLogDao } = await getServerDiContainer();
      ...
      if (otherQuery) {
        const { data, count } = await createUserLogDao.list(paginator, filter);
        res.status(200).json({ logs: data, total: count });
      } else {
        const { data, count } = await createUserLogDao.list(paginator);
        res.status(200).json({ logs: data, total: count });
      }
    });
  } catch (error) {
