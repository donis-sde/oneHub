...
const rbacMiddleware = createRbacMiddleware(
  rbacRules,
  BackofficeFeature.TERMINATE_SUBSCRIPTION_LOGS,
);
...
export default async function handler(
  ...
  try {
    await rbacMiddleware(req, res, async () => {
      const { terminateSubscriptionLogDao } = await getServerDiContainer();
      ...
      if (otherQuery) {
        const { data, count } = await terminateSubscriptionLogDao.list(
          paginator,
          filter,
        );
