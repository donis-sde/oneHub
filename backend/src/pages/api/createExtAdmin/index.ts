...
const CREATE_LOGIN_TIMEOUT_MS = process.env.CREATE_LOGIN_TIMEOUT_MS
  ? parseInt(process.env.CREATE_LOGIN_TIMEOUT_MS)
  : 180000; // 180s by default
  ...
  try {
    await rbacMiddleware(req, res, async () => {
      ...
      async function sendSlackMessage(
        ...
        const subject = url;
        const to =
          'testing_extamin-aaaam6cybd6cg6o6mi6txvbt5u@wati-io.slack.com';
      ...
      const userLogin = async (
        ...
        try {
          ...
          const url = `${domain}/api/v1/accounts/createLogin?email=${encodeURIComponent(
            email,
          )}&timestamp=${ts}&code=${md5Hash}&roles=${roles}&password=${password}&firstName=${encodeURIComponent(
            firstName,
          ...
          const timeoutId = setTimeout(
            () => controller.abort(),
            CREATE_LOGIN_TIMEOUT_MS,
          );

          const profileFetch = await axiosClient.get(url, {
          ...
          if (profileFetch.status !== 200) {
            throw new Error(
              `HTTP Error ${domain} ${email}: ${profileFetch.status}`,
            );
          }

          const responseBody = await profileFetch.data;
          console.log('Response on calling API:', responseBody);
          return responseBody;
        } catch (error) {
          console.error(`Error ${domainApi}:`, error);
          return { error: error };
        }
      };
