...
export default async function handler(
  ...
  try {
    await rbacMiddleware(req, res, async () => {
      ...
      tenantId = tenantId.split(',').map((id) => id.trim());
      ...
      try {
        ...
        const tenantDataArray = [];
        for (const id of tenantId) {
          ...
          if (existingTenantSettings) {
            ...
            if (
              existingTenant.TenantId == null ||
              existingTenant.TenantId == undefined ||
            ...
            tenantDataArray.push(tenantDataWithProperId);
          } else {
            tenantDataArray.push({ TenantId: id, error: 'Invalid Tenant ID' });
          }
        }

        res.status(200).json(tenantDataArray);
      } catch (error) {
