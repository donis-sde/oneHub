...
export type RetoolTenantsListResponse = Paginated<WithId<TenantDto>>;
export type RetoolTenantsFindOneAndUpdateResponse = {
  success: boolean;
  data: WithId<TenantDto> | null;
};

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<
    RetoolTenantsListResponse | RetoolTenantsFindOneAndUpdateResponse
  >,
): Promise<void> {
  ...
  try {
    ...
    >([
      ...
      async (
        ...
        const loggerMetadata = {
          ...req.logContext,
          functionName: 'retoolTenantsHandler',
          API: 'RetoolTenants',
        };

        logger.debug('Retool tenants request', {
          method: req.method,
          query: req.query,
          body: req.body,
          loggerMetadata,
        ...
        if (req.method === HttpMethod.GET) {
          // Handle GET - List tenants with filters
          ...
          const result = await tenantDao.list(paginator, filters as any);
          ...
          let updatedTenant: WithId<TenantDto> | null = null;
          let fieldChanged = '';
          let newValue = '';
          ...
          if (updates && typeof updates === 'object') {
            ...
            updatedTenant = await tenantDao.findAndUpdateOneMultiple(
              filters,
              updates,
            );
            ...
            updatedTenant = await tenantDao.findAndUpdateOne(
              filters,
              field as keyof TenantDto,
              value,
            );
            fieldChanged = field;
          ...
          await backofficeDbUpdateLogsDao.insertMany([
            {
              collectionName: 'Tenant',
              oldValue: 'N/A',
              impactedRecordObjectId: updatedTenant?._id?.toString() || 'N/A',
              tenantId: 'N/A',
              impactedRecordEmail: 'N/A',
              dbName: 'TENANTS_DATABASE',
              changedBy: 'retool@api',
    ...
    logger.error('/api/retool/tenants:error', { err: `${err}` });
    res.status(500).json({
      data: [],
